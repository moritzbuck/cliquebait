import argparse
import sys
sys.path += ['/home/moritz/projects/0079_pelaginet/']
print(sys.path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--denoising_cutoff', '-d', nargs = 1, type=float, default = default_denoising_cutoff, help = f"Similarities of a branch are ordered in decreasing order above that threshold are ignored in the clustering (considered intra-species clusters, e.g. strain), default {default_lower_cutoff}")
    ...
    if len(sys.argv)==1 or args.version:
        ...
        print("{script} Version {version}".format(script = __file__, version = version))
        sys.exit(1)
